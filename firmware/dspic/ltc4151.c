...
float ltc4151_read_voltage()
{
	...
	error = I2C_Receive(LTC4151_WRITE,LTC4151_READ,LTC4151_VIN_LSB,&data_LSB,I2C_1);
	if (error == -1)
		{
			return (float)error;
		}

	error = I2C_Receive(LTC4151_WRITE,LTC4151_READ,LTC4151_VIN_MSB,&data_MSB,I2C_1);
		if (error == -1)
		{
			return (float)error;
		}
